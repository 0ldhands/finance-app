...
import '../css/Home.css'
  ...
  const Incgraph = ({incg}) => {
    ...
    const add=()=>{
      let casfil=incg.filter((crr)=>crr.payment == 'Cash')
      let sum1=[0,...casfil]
      let castot=sum1.reduce((tot,pre)=>tot+=Number(pre.amount))
      setCash(()=>castot)

      let upifil=incg.filter((crr)=>crr.payment == 'Upi')
      let sum2=[0,...upifil]
      let upitot=sum2.reduce((tot,pre)=>tot+=Number(pre.amount))
      setUpi(()=>upitot)

      let carfil=incg.filter((crr)=>crr.payment == 'Card')
      let sum3=[0,...carfil]
